...
int put_elf_sym(Section *s, unsigned long value, unsigned long size, int info,
...
int add_elf_sym(Section *s, unsigned long value, unsigned long size, int info,
		int other, int sh_num, const char *name)
{
	ElfW(Sym) *esym;
	...
	if (sym_bind != STB_LOCAL)
	{
...
#if 1
				printf(
						"new_bind=%x new_shndx=%x new_vis=%x old_bind=%x old_shndx=%x old_vis=%x\n",
						sym_bind, sh_num, new_vis, esym_bind, esym->st_shndx,
						esym_vis);
#endif
		...
		{
			do_patch: esym->st_info = ELFW(ST_INFO)(sym_bind, sym_type);
	...
	{
		do_def: sym_index = put_elf_sym(s, value, size,
				ELFW(ST_INFO)(sym_bind, sym_type), other, sh_num, name);
	}
	return sym_index;
}

/* put relocation */
void put_elf_reloc(Section *symtab, Section *s, unsigned long offset, int type,
		int symbol)
{
	char buf[256];
	Section *sr;
		...
		unsigned long value, Section *sec, int sym_index)
{
	...
	put_elf_reloc(symtab_section, stab_section, stab_section->data_offset
			- sizeof(unsigned int), R_DATA_32, sym_index);
}
...
void put_got_entry(TCCState *s1, int reloc_type, unsigned long size, int info,
		int sym_index)
{
	int index;
	const char *name;
...
#error unsupported CPU
		...
		index = put_elf_sym(s1->dynsym, offset, size, info, 0, sym->st_shndx,
				name);
		/* put a got entry */
		put_elf_reloc(s1->dynsym, s1->got, s1->got->data_offset, reloc_type,
				index);
	}
...
Section *new_symtab(TCCState *s1, const char *symtab_name, int sh_type,
...
#ifdef __FreeBSD__
char elf_interp[] = "/usr/libexec/ld-elf.so.1";
#else
#ifdef TCC_ARM_EABI
char elf_interp[] = "/lib/ld-linux.so.3";
#elif defined(TCC_TARGET_X86_64)
char elf_interp[] = "/lib/ld-linux-x86-64.so.2";
#else
char elf_interp[] = "/lib/ld-linux.so.2";
#endif
#endif
...
int elf_output_file(TCCState *s1, const char *filename)
{
	...
	if (file_type != TCC_OUTPUT_OBJ)
	{
		...
		if (!s1->static_link)
		{
			...
			/* add dynamic section */
			dynamic = new_section(s1, ".dynamic", SHT_DYNAMIC, SHF_ALLOC
					| SHF_WRITE);
			dynamic->link = dynstr;
			dynamic->sh_entsize = sizeof(ElfW(Dyn));

			/* add PLT */
			s1->plt = new_section(s1, ".plt", SHT_PROGBITS, SHF_ALLOC
					| SHF_EXECINSTR);
			s1->plt->sh_entsize = 4;
			...
			if (file_type == TCC_OUTPUT_EXE)
			{
				for (sym = (ElfW(Sym) *) symtab_section->data + 1; sym
						< sym_end; sym++)
				{
					if (sym->st_shndx == SHN_UNDEF)
					{
						...
						{
							esym
									= &((ElfW(Sym) *) s1->dynsymtab_section->data)[sym_index];
							...
							else if (type == STT_OBJECT)
							{
								...
								index = put_elf_sym(s1->dynsym, offset,
										esym->st_size, esym->st_info, 0,
										bss_section->sh_num, name);
								put_elf_reloc(s1->dynsym, bss_section, offset,
										R_COPY, index);
								offset += esym->st_size;
								bss_section->data_offset = offset;
							}
					...
					else if (s1->rdynamic && ELFW(ST_BIND)(sym->st_info)
							!= STB_LOCAL)
					{
				...
				for (esym = (ElfW(Sym) *) s1->dynsymtab_section->data + 1; esym
						< sym_end; esym++)
				{
					if (esym->st_shndx == SHN_UNDEF)
					{
						...
						if (sym_index)
						{
							...
							sym
									= &((ElfW(Sym) *) symtab_section->data)[sym_index];
							put_elf_sym(s1->dynsym, sym->st_value,
			...
			else
			{
						...
						< sym_end; sym++)
				{
					if (ELFW(ST_BIND)(sym->st_info) != STB_LOCAL)
					{
						...
						{
							name = symtab_section->link->data + sym->st_name;
							index = put_elf_sym(s1->dynsym, sym->st_value,
									sym->st_size, sym->st_info, 0,
									sym->st_shndx, name);
			...
			for (i = 0; i < s1->nb_loaded_dlls; i++)
			{
				...
				if (dllref->level == 0)
					put_dt(dynamic, DT_NEEDED,
							put_elf_str(dynstr, dllref->name));
			}
	...
	for (i = 1; i < s1->nb_sections; i++)
	{
		...
		else if (do_debug || file_type == TCC_OUTPUT_OBJ || (s->sh_flags
				& SHF_ALLOC) || i == (s1->nb_sections - 1))
		{
		...
		for (j = 0; j < 2; j++)
		{
			...
			for (k = 0; k < 5; k++)
			{
				for (i = 1; i < s1->nb_sections; i++)
				{
					...
					else if (s->sh_type == SHT_DYNSYM || s->sh_type
							== SHT_STRTAB || s->sh_type == SHT_HASH)
					{
						if (k != 1)
							continue;
					}
...
#error unsupported CPU
			...
			sym_end
					= (ElfW(Sym) *) (s1->dynsym->data + s1->dynsym->data_offset);
	...
	for (i = 1; i < s1->nb_sections; i++)
	{
		...
		file_offset = (file_offset + s->sh_addralign - 1) & ~(s->sh_addralign
				- 1);
		s->sh_offset = file_offset;
		if (s->sh_type != SHT_NOBITS)
			file_offset += s->sh_size;
	...
	ret = 0;
	the_end: tcc_free(s1->symtab_to_dynsym);
...
int tcc_load_object_file(TCCState *s1, int fd, unsigned long file_offset)
{
	...
	{
		fail1: error_noabort("invalid object file");
		return -1;
	}
	/* read sections */
	shdr = load_data(fd, file_offset + ehdr.e_shoff, sizeof(ElfW(Shdr))
			* ehdr.e_shnum);
	...
	for (i = 1; i < ehdr.e_shnum; i++)
	{
		...
		for (j = 1; j < s1->nb_sections; j++)
		{
			...
			{
				if (!strncmp(sh_name, ".gnu.linkonce", sizeof(".gnu.linkonce")
						- 1))
				{
		...
		no_align: sm_table[i].offset = offset;
		sm_table[i].s = s;
	...
	for (;;)
	{
		...
		if (!strcmp(ar_name, "/"))
		{
			...
			if (s1->alacarte_link)
				...
				|| !strcmp(ar_name, "__.SYMDEF/") || !strcmp(ar_name,
				"ARFILENAMES/"))
		{
...
int tcc_load_dll(TCCState *s1, int fd, const char *filename, int level)
{
	...
	for (i = 0, dt = dynamic; i < nb_dts; i++, dt++)
	{
		...
		case DT_NEEDED:
			...
			already_loaded: break;
		}
	}
	ret = 0;
	the_end: tcc_free(dynstr);
	tcc_free(dynsym);
	tcc_free(dynamic);
	tcc_free(shdr);
	return ret;
