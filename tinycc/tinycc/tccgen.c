...
Sym *get_sym_ref(CType *type, Section *sec, unsigned long offset,
		unsigned long size)
{
	int v;
	Sym *sym;
...
void vpush_ref(CType *type, Section *sec, unsigned long offset,
		unsigned long size)
{
	CValue cval;
...
Sym *external_sym(int v, CType *type, int r)
{
	...
	else
	{
		if (!is_compatible_types(&s->type, type))
			error("incompatible types for redefinition of '%s'", get_tok_str(v,
					NULL));
	}
	return s;
}
...
void save_reg(int r)
{
	...
	{
		if ((p->r & VT_VALMASK) == r || ((p->type.t & VT_BTYPE) == VT_LLONG
				&& (p->r2 & VT_VALMASK) == r))
		{
			...
			if (!saved)
			{
				...
				type = &p->type;
				if ((p->r & VT_LVAL) || (!is_float(type->t) && (type->t
						& VT_BTYPE) != VT_LLONG))
...
int gv(int rc)
{
	...
	if (vtop->type.t & VT_BITFIELD)
	{
		...
		if ((vtop->type.t & VT_BTYPE) == VT_LLONG)
		{
			...
			type.t = VT_INT;
		if ((vtop->type.t & VT_UNSIGNED) || (vtop->type.t & VT_BTYPE)
				== VT_BOOL)
			type.t |= VT_UNSIGNED;
		gen_cast(&type);
		/* generate shifts */
	...
	{
		if (is_float(vtop->type.t) && (vtop->r & (VT_VALMASK | VT_LVAL))
				== VT_CONST)
		{
			Sym *sym;
			int *ptr;
...
void gen_opic(int op)
{
	...
	else
	{
		...
		if (c2
				&& (((op == '*' || op == '/' || op == TOK_UDIV || op
						== TOK_PDIV) && l2 == 1) || ((op == '+' || op == '-'
						|| op == '|' || op == '^' || op == TOK_SHL || op
						== TOK_SHR || op == TOK_SAR) && l2 == 0) || (op == '&'
						&& l2 == -1)))
		{
			/* nothing to do */
			vtop--;
		...
		else if (c2 && (op == '+' || op == '-') && ((vtop[-1].r & (VT_VALMASK
				| VT_LVAL | VT_SYM)) == (VT_CONST | VT_SYM) || (vtop[-1].r
				& (VT_VALMASK | VT_LVAL)) == VT_LOCAL))
		{
...
static inline int is_null_pointer(SValue *p)
{
	if ((p->r & (VT_VALMASK | VT_LVAL | VT_SYM)) != VT_CONST)
		return 0;
	return ((p->type.t & VT_BTYPE) == VT_INT && p->c.i == 0) || ((p->type.t
			& VT_BTYPE) == VT_LLONG && p->c.ll == 0);
}
...
void check_comparison_pointer_types(SValue *p1, SValue *p2, int op)
{
	...
	{
		invalid_operands: error("invalid operands to binary %s", get_tok_str(
				op, NULL));
	}
...
void gen_op(int op)
{
	...
	else if (bt1 == VT_LLONG || bt2 == VT_LLONG)
	{
		...
		if ((t1 & (VT_BTYPE | VT_UNSIGNED)) == (VT_LLONG | VT_UNSIGNED) || (t2
				& (VT_BTYPE | VT_UNSIGNED)) == (VT_LLONG | VT_UNSIGNED))
			t |= VT_UNSIGNED;
		goto std_op;
	}
	else
	{
		...
		if ((t1 & (VT_BTYPE | VT_UNSIGNED)) == (VT_INT | VT_UNSIGNED) || (t2
				& (VT_BTYPE | VT_UNSIGNED)) == (VT_INT | VT_UNSIGNED))
			t |= VT_UNSIGNED;
		std_op:
		...
		else if (!nocode_wanted)
		{
			...
			else if (sf)
			{
				...
				else
				{
					...
					if (dbt != (VT_INT | VT_UNSIGNED) && dbt != (VT_LLONG
							| VT_UNSIGNED) && dbt != VT_LLONG)
						dbt = VT_INT;
					gen_cvt_ftoi1(dbt);
...
void type_to_str(char *buf, int buf_size, CType *type, const char *varstr)
{
	...
	case VT_LDOUBLE:
		tstr = "long double";
		add_tstr: pstrcat(buf, buf_size, tstr);
		break;
	case VT_ENUM:
	case VT_STRUCT:
		if (bt == VT_STRUCT)
...
void gen_assign_cast(CType *dt)
{
	...
	case VT_PTR:
		...
		if (sbt == VT_FUNC)
		{
			if ((type1->t & VT_BTYPE) != VT_VOID && !is_compatible_types(
					pointed_type(dt), st))
				goto error;
			else
				goto type_ok;
		...
		if ((type1->t & VT_BTYPE) == VT_VOID || (type2->t & VT_BTYPE)
				== VT_VOID)
		{
	...
	case VT_STRUCT:
		...
		if (!is_compatible_types(&tmp_type1, &tmp_type2))
		{
			error: type_to_str(buf1, sizeof(buf1), st, NULL);
...
void vstore(void)
{
	...
	dbt = ft & VT_BTYPE;
	if (((sbt == VT_INT || sbt == VT_SHORT) && dbt == VT_BYTE) || (sbt
			== VT_INT && dbt == VT_SHORT))
	{
		/* optimize char/short casts */
		delayed_cast = VT_MUSTCAST;
		vtop->type.t = ft
				& (VT_TYPE & ~(VT_BITFIELD | (-1 << VT_STRUCT_SHIFT)));
		/* XXX: factorize */
		if (ft & VT_CONSTANT)
			warning("assignment of read-only location");
	}
				...
				while (1)
				{
					...
					if (tok != ':')
					{
						...
						if (v == 0 && (type1.t & VT_BTYPE) != VT_STRUCT)
							expect("identifier");
						if ((type1.t & VT_BTYPE) == VT_FUNC || (type1.t
								& (VT_TYPEDEF | VT_STATIC | VT_EXTERN
										| VT_INLINE)))
					...
					if (tok == ':')
					{
						...
						if (bit_size < 0)
							error("negative width in bit-field '%s'",
									get_tok_str(v, NULL));
						if (v && bit_size == 0)
							error("zero width for bit-field '%s'", get_tok_str(
									v, NULL));
					}
					...
					if (bit_size >= 0)
					{
						...
						else
						{
							...
							type1.t |= VT_BITFIELD | (bit_pos
									<< VT_STRUCT_SHIFT) | (bit_size
									<< (VT_STRUCT_SHIFT + 6));
							bit_pos += bit_size;
						}
						prevbt = bt;
...
int parse_btype(CType *type, AttributeDef *ad)
{
	...
	the_end: if ((t & (VT_SIGNED | VT_UNSIGNED)) == (VT_SIGNED | VT_UNSIGNED))
...
void unary(void)
{
	...
	case TOK_STR:
		/* string parsing */
		t = VT_BYTE;
		str_init: if (tcc_state->warn_write_strings)
			t |= VT_CONSTANT;
		type.t = t;
		mk_pointer(&type);
		type.t |= VT_ARRAY;
	...
	default:
		...
		if (!s)
		{
			...
			if (tcc_state->warn_implicit_function_declaration)
				warning("implicit declaration of function '%s'", get_tok_str(t,
						NULL));
			s = external_global_sym(t, &func_old_type, 0);
		}
		if ((s->type.t & (VT_STATIC | VT_INLINE | VT_BTYPE)) == (VT_STATIC
				| VT_INLINE | VT_FUNC))
		{
	...
	while (1)
	{
		...
		else if (tok == '.' || tok == TOK_ARROW)
		{
			...
			if (!s)
				error("field not found: %s",
						get_tok_str(tok & ~SYM_FIELD, NULL));
		...
		else if (tok == '(')
		{
			...
			if ((vtop->type.t & VT_BTYPE) != VT_FUNC)
			{
				...
				{
					error_func: expect("function pointer");
				}
			}
			else
			{
...
void uneq(void)
{
	...
	unary();
	if (tok == '=' || (tok >= TOK_A_MOD && tok <= TOK_A_DIV) || tok
			== TOK_A_XOR || tok == TOK_A_OR || tok == TOK_A_SHL || tok
			== TOK_A_SAR)
	{
		test_lvalue();
		t = tok;
...
void expr_cmp(void)
{
	...
	expr_shift();
	while ((tok >= TOK_ULE && tok <= TOK_GT) || tok == TOK_ULT || tok
			== TOK_UGE)
	{
		t = tok;
		next();
		...
		if (tok == '?')
		{
			...
			else if (bt1 == VT_LLONG || bt2 == VT_LLONG)
			{
				...
				if ((t1 & (VT_BTYPE | VT_UNSIGNED)) == (VT_LLONG | VT_UNSIGNED)
						|| (t2 & (VT_BTYPE | VT_UNSIGNED)) == (VT_LLONG
								| VT_UNSIGNED))
					type.t |= VT_UNSIGNED;
			}
			...
			else
			{
				...
				if ((t1 & (VT_BTYPE | VT_UNSIGNED)) == (VT_INT | VT_UNSIGNED)
						|| (t2 & (VT_BTYPE | VT_UNSIGNED)) == (VT_INT
								| VT_UNSIGNED))
					type.t |= VT_UNSIGNED;
			}
		...
		int *cur_index, Sym **cur_field, int size_only)
{
	...
	while (tok == '[' || tok == '.')
	{
		...
		else
		{
			...
			l = tok;
			next();
			struct_field: if ((type->t & VT_BTYPE) != VT_STRUCT)
...
void init_putv(CType *type, Section *sec, unsigned long c, int v, int expr_type)
{
	...
	if (sec)
	{
		...
		if ((vtop->r & VT_SYM) && (bt == VT_BYTE || bt == VT_SHORT || bt
				== VT_DOUBLE || bt == VT_LDOUBLE || bt == VT_LLONG || (bt
				== VT_INT && bit_size != 32)))
...
void decl_initializer(CType *type, Section *sec, unsigned long c, int first,
		int size_only)
{
	...
	if (type->t & VT_ARRAY)
	{
		...
		else
		{
			...
			while (tok != '}')
			{
				...
				{
					init_putz(t1, sec, c + array_length * size1, (index
							- array_length) * size1);
				}
				index++;
		...
		{
			init_putz(t1, sec, c + array_length * size1, (n - array_length)
					* size1);
		}
...
void decl_initializer_alloc(CType *type, AttributeDef *ad, int r, int has_init,
		int v, int scope)
{
	...
	if ((r & VT_VALMASK) == VT_LOCAL)
	{
		...
		if (do_bounds_check && (type->t & VT_ARRAY))
		{
			...
			bounds_ptr = section_ptr_add(lbounds_section, 2
...
void func_decl_list(Sym *func_sym)
{
	...
	while (tok != '{' && tok != ';' && tok != ',' && tok != TOK_EOF)
	{
		...
		if (((btype.t & VT_BTYPE) == VT_ENUM || (btype.t & VT_BTYPE)
				== VT_STRUCT) && tok == ';')
		{
...
void gen_inline_functions(void)
{
	...
	for (;;)
	{
		...
		{
			type = &sym->type;
			if (((type->t & VT_BTYPE) == VT_FUNC) && (type->t & (VT_STATIC
					| VT_INLINE)) == (VT_STATIC | VT_INLINE) && sym->c != 0)
			{
...
void decl(int l)
{
	...
	while (1)
	{
		if (!parse_btype(&btype, &ad))
		{
			...
			if (l == VT_CONST && (tok == TOK_ASM1 || tok == TOK_ASM2 || tok
					== TOK_ASM3))
			{
				/* global asm block */
				asm_global_instr();
		...
		{ /* iterate thru each declaration */
			...
			if (tok == '{')
			{
				...
				if (sym)
				{
					...
					r = sym->type.ref->r;
					if (FUNC_CALL(r) != FUNC_CDECL && FUNC_CALL(type.ref->r)
							== FUNC_CDECL)
					...
					{
						func_error1: error(
								"incompatible types for redefinition of '%s'",
								get_tok_str(v, NULL));
					}
				...
				if ((type.t & (VT_INLINE | VT_STATIC)) == (VT_INLINE
						| VT_STATIC))
				{
			...
			else
			{
				...
				else
				{
					...
					if ((btype.t & VT_EXTERN) || ((type.t & VT_ARRAY)
							&& (type.t & VT_STATIC) && !has_init && l
							== VT_CONST && type.ref->c < 0))
					{
						/* external variable */
